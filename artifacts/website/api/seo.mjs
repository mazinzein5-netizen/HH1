/**
 * SEO meta injection logic — extracted from server.mjs for Vercel serverless.
 * Reads the built index.html, injects route-specific SEO tags, serves the result.
 */
...
export function injectMeta(html, meta, robots) {
  return html
...
export function handleRequest(req) {
  ...
  const clean = normalizePath(urlPath);
  if (urlPath.length > 1 && urlPath.endsWith("/") && !urlPath.startsWith("/api")) {
    const target = urlPath.replace(/\/+$/, "") || "/";
    const query = urlPath.includes("?") ? urlPath.slice(urlPath.indexOf("?")) : "";
    return {
      statusCode: 301,
      headers: { Location: target + query },
      body: "",
    };
  }
  ...
  try {
    ...
    return {
      ...
      body: "index.html not found. Build may have failed.",
    };
  }

  const { meta, status, robots } = resolveRoute(urlPath);
