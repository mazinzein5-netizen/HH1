...
There is no platform/sandbox callback that exports a video artifact. A reusable recorder lives at `scripts/record-promo.mjs` (puppeteer-core + the nix-store playwright Chromium + CDP `Page.startScreencast`), followed by ffmpeg concat encoding.
